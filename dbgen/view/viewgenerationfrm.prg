...
********************************* END METHOD **********************************
*******************************************************************************
*/LV FRM: список типов документов котоми можно можно оплатить текущий документ/*
PROCEDURE lvFrmTypeListPayByFrm
***
CREATE SQL VIEW lvFrmTypeListPayByFrm REMOTE CONNECTION 'SQLBASISCONNECTION' SHARE AS ;
SELECT FrmTypeTo.FrmTypeID ;
    , FrmTypeTo.FrmTypeNM ;
    , '' AS CopyOperProcType ;
    , '' AS CopyOperProcNM ;
    , CASE WHEN FrmTypePay.IsDefault = 1 ;
                THEN 1 ELSE 0 END AS default_ ;
FROM Form AS FormFrom ;
INNER JOIN FrmTypePay ON FrmTypePay.IncludeFrmTypeID = FormFrom.FrmTypeID ;
INNER JOIN FrmType AS FrmTypeTo ON FrmTypeTo.FrmTypeID = FrmTypePay.FrmTypeID ;
WHERE FormFrom.FrmID = ?_PARAM ;
ORDER BY default_ DESC, FrmTypeTo.FrmTypeNM
***
DBSETPROP([lvFrmTypeListPayByFrm],[VIEW],[FetchSize],-1)
***
DBSETPROP([lvFrmTypeListPayByFrm],[VIEW],[COMMENT],[LV FRM: список типов документов котоми можно можно оплатить текущий документ])
***
ENDPROC
*------------------------------------------------------------------------------    
*******************************************************************************
